...
public class LargeScaleTest {
	...
	final int nOutputGroupsMax=1000;
	final int nVariableIDsMax=1000;
	int nOutputGroups;
	...
	public  LargeScaleTest(){
		...
		key[0]="***  Power Sources";
		key[1]="***  Sources ";
		key[2]="*            Magnetic fluxes of flux loops";
		key[3]="*** Total magnetic";
		key[4]="*** Total Joule heat";
	...
	private void compareOutputsAndWrite(){
		...
		String[] ff=null;
		String[] tip=null;
		String[][] outItems1=new String[nOutputGroupsMax][];
		int nOutputItems=0;
		try{	
			...
			for(int i=0;i<outItems1.length;i++){
				line=br.readLine();
				...
				outItems1[i]=new String[sp.length-ib];

				for(int j=0;j<outItems1[i].length;i++){
					outItems1[i][j]=sp[ib+i];
				}
		...
		outItemIds=new String[nOutputItems][];

		for(int i=0;i<outItemIds.length;i++){
			if(outItems1[i]!=null)
				outItemIds[i]=outItems1[i];


		}
		outItems1=null;

		nOutputGroups=outItemIds.length;
		...
		stepNumbs=new int[nFiles][10000];
		time=	new Vect[nFiles];

		output=new String[nFiles][nOutputGroups][][][];
		outputID=new String[nFiles][nOutputGroups][][];

		FileWriter	ferr=null;
		PrintWriter  pwerr=null;

		try{
			ferr=new FileWriter("stderr");		
			...
			for(int nfile=0;nfile<nFiles;nfile++){
				...
				{
					pwerr.println();
					pwerr.println("Failed in reading timesteps in file "+file[nfile]);
					pwerr.println("Please check the file.");
				...
				for(int i=0;i<nOutputGroups;i++){
					//if(i>0) break;
					this.loadOutputGroup(nfile,i);
				}

			}
		}
		catch(IOException e)
		{  pwerr.print(e.getMessage());
		}

		int nf=0;

		System.out.println(output.length+"  "+output[0].length+" "+output[0][0].length+" "+output[0][0][0].length+" "+output[0][0][0][0].length);
		
		for(int p=0;p<nOutputGroups;p++){
			for(int j=0;j<output[nf][p][0].length;j++){
				for(int k=0;k<output[nf][p][0][j].length;k++)
					System.out.print("\t"+output[nf][p][0][j][k]);
					//System.out.format("%15s",output[nf][i][j][k][0]);
				System.out.println();
			}
...

	{
		...
		String[][][] fieldFlux=new String[nFiles][][];
		...
		String[][][][] table=new String[nFiles][][][];
		...
		if(time[nfile]==null)
		{
			...
			compWriter[nfile].println("Failed in reading timesteps in file "+file[nfile]);
		compWriter[nfile].println("Please check the file.");

		sumWriter.println();
		sumWriter.println("Failed in reading timespets in file "+file[nfile]);
		...
		//	int nMax=100;
		//	if(outItems.length>0) nMax=outItems.length;
		...
		//--------------------------------------------
			...
			fieldSourceCurrent[nfile]=new String[nFieldSources[nfile]][nT];
				...
				fieldFlux[nfile]=new String[nFieldSources[nfile]][nT];

				for(int i=0;i<nFieldSources[nfile];i++){


					fieldFlux[nfile][i]=loadCurFluxString(file[nfile],i+1,dummy);


			}


				if(fieldFlux[nfile][0]!=null) 
	...
	for(int t=0;t<nT;t++){
		...
		table[nfile][iGroup][t][ix++]=Double.toString(time[nfile].el[t]);
					...
					if(nCurFluxes[nfile]>0)
						for(int i=0;i<nFieldSources[nfile];i++){
							table[nfile][iGroup][t][ix++]=fieldFlux[nfile][i][t];	
	...
	public String[] loadCurFluxString(String file, int sourceIndex,String[] id){
		...
		String[] fieldFlux1=new String[10000];
		...
		try{
			...
			String[] fieldFlux=new String[ix];

			for(int i=0;i<ix;i++){
				fieldFlux[i]=fieldFlux1[i];
	...
	private void loadOutputGroup(int nfile ,int groupIndex){



		String[][] data1=new String[nTmax][nVariableIDsMax];
		String[] id;
		...
		int nVarsID=0;
		try{
			FileReader fr=new FileReader(file[nfile]);
			BufferedReader br = new BufferedReader(fr);
			String line;

			while((line=br.readLine())!=null){
				while((line=br.readLine())!=null && !line.startsWith(key[groupIndex])){}
				if(line==null) {break;}

				line=br.readLine();
		
				nVarsID=0;
				for(int i=0;i<nVariableIDsMax;i++){
					line=br.readLine();
	
					if(line.startsWith("*") || line.equals("") || line==null)
						break;

				data1[ix][nVarsID]=line;
		...
		id=new String[nVarsID];
		 int ib=0;
        
		 int nInGroups=0;
		String[]  sp1=null;
		for(int j=0;j<nVarsID;j++){
		 sp1=data1[j][0].split(regex);

		 if(sp1[0].equals("")) ib=1;
		 id[j]=sp1[ib];
		 if(j==0) nInGroups=sp1.length-1-ib;
		}
		

		

			int inVarOuputs=nVarsID;
			
			if(outItemIds.length>0){
				
				if(outItemIds[groupIndex].length>0){
				inVarOuputs=outItemIds[groupIndex].length;
			...
			int[] varIdMap=new int[inVarOuputs];
			
			if(inVarOuputs==nVarsID){
				for(int j=0;j<nVarsID;j++)
					varIdMap[j]=j;
			}
			else{

				for(int i=0;i<inVarOuputs;i++){
					for(int j=0;j<nVarsID;j++){
						if(id[j].equals(outItemIds[groupIndex][i])){
							varIdMap[i]=j;
							break;
						}
					}
			...
			output[nfile][groupIndex]=new String[ix][inVarOuputs][nInGroups];
			//if(ix==0) return null;


			for(int i=0;i<ix;i++){

				for(int j=0;j<inVarOuputs;j++){
			
					 String[]  sp=data1[i][varIdMap[j]].split(regex);
					
					 int ib1=0;
					 if(sp[0].equals("")) ib1=1;
	
					 for(int k=0;k<nInGroups;k++){
					output[nfile][groupIndex][i][j][k]=sp[ib1+1+k];
					 }
